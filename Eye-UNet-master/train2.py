...
# dir_img = Path('/sdata/wenhui.ma/program/dataset/mobi_seg/train/image')
# dir_mask = Path('/sdata/wenhui.ma/program/dataset/mobi_seg/train/label')
...
dir_checkpoint = Path('./'+f'{dataset_name}'+'_checkpoints/model/')
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    masks_pred1 = model(images)['output']
                    masks_pred2 = model(images)['output_mini']
                    if model.n_classes == 1:
                        loss = criterion(masks_pred1.squeeze(1), true_masks1.float())
                        loss += 0.5*criterion(masks_pred2.squeeze(1), true_masks2.float())
                        # loss += dice_loss(F.sigmoid(masks_pred.squeeze(1)), true_masks.float(), multiclass=False)
                    else:
                        loss = criterion(masks_pred1, true_masks1)
                        loss += 0.5*criterion(masks_pred2, true_masks2)
                        # loss += dice_loss(
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        ...
                        val_iou = evaluate(model, val_loader, device, amp)
                        ...
                        try:
                            experiment.log({
                                ...
                                'masks': {
                                    'true': wandb.Image(true_masks1[0].float().cpu()),
                                    'pred': wandb.Image(masks_pred1.argmax(dim=1)[0].float().cpu()),
                                },
                                'step': global_step,
                                'epoch': epoch,
                                **histograms
...
if __name__ == '__main__':
    ...
    args.epochs = 200
    args.batch_size = 24
    model = Eye_UNet(n_channels=3, n_classes=args.classes)
