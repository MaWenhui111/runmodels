...
class BasicDataset(Dataset):
    def __init__(self, images_dir: str, mask_dir: str, scale: float = 1.0, deep_mask=False, num_classes=4):
    ...
    def preprocess(mask_values, img, scale, is_mask):
        ...
        if is_mask:
            ...
            return img, Iris, sclera, pupil

    def __getitem__(self, idx):
        name = self.ids[idx]
        mask_file = glob(self.mask_dir + name + '.*')
        img_file = glob(self.images_dir + name + '.*')
        ...
        img, Iris, sclera, pupil = self.preprocess(self.mask_values, img, self.scale, is_mask=False)  # 由h,w,c变为c,h,w
        cv2.imwrite('/sdata/wenhui.ma/program/models/SFNet-master/test/' + name + '.png', mask)
        ...
        # sclera_mask = cv2.resize(sclera_mask, (10, 18), interpolation=cv2.INTER_AREA)
        # Iris_mask = cv2.resize(Iris_mask, (10, 18), interpolation=cv2.INTER_AREA)
        # pupil_mask = cv2.resize(pupil_mask, (10, 18), interpolation=cv2.INTER_AREA)

        return sclera, Iris, pupil, sclera_mask, Iris_mask, pupil_mask, mask, name
        # if self.deep_mask:
...
if __name__ == '__main__':
    img_scale = 0.6
    batch_size = 24
    dir_img = '/sdata/wenhui.ma/program/dataset/Mobi-Seg/test/image/'
    dir_mask = '/sdata/wenhui.ma/program/dataset/Mobi-Seg/test/label/'
    ...
    for batch in dataset_loader:
        sclera, Iris, pupil, sclera_mask, Iris_mask, pupil_mask, mask, name = batch
